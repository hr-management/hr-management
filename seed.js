...
db.once("open", async function () {
  ...
  try {
    ...
    const user = await userModel.findOne({ username: 'Employee1' });

    // Check if the user exists and if the assignedHouse field is not present
    if(user) {
      user.assignedHouse = { _id: new mongoose.Types.ObjectId('645de4285b1de6a2494a9150') };
      const updatedUser = await user.save();

      console.log(updatedUser);
    }

    // Find the user by ObjectId and populate the assignedHouse field
    const userWithHouse = await userModel
      .findOne({ _id: '645b477a6b91983b4967bbda' })
      .populate('assignedHouse._id');

    // Log the user document with the populated house data
    console.log(userWithHouse);
  } catch(err) {
